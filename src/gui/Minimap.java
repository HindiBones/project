...
public class Minimap extends JPanel {
	...
	public Minimap(HindiBones fenster) {
		this.fenster = fenster;
		 ...
		 ChatText = new JTextField(); //Erzeugen eines Textfeldes für Nicknamen
	...
	public void paint(Graphics g) {
		...
		//Item Hintergrund Kästchen 3x (Falls später noch ein Item hinzugefügt wird)
		g.setColor(Color.GRAY);
		...
		if (feld instanceof Schluessel) {
			...
			g.drawString(" Nimm den Schlüssel", 7, 400);
		} else if (feld instanceof Tuer) {
			if (!((Tuer) feld).istOffen()) {
				
				if (fenster.spieler.hatSchluessel()){
					g.drawImage(sblase,0,375,null);
					g.drawString("Öffne die Tür",7, 400);}
				else{
					g.drawImage(sblase,0,375,null);
					g.drawString("Tür ist verschlossen!", 7, 400);}
			}
		...
		this.add(ChatText);
		 ChatText.setBounds(0,410,230,35);//Größe+Koord. wird festgelegt 
