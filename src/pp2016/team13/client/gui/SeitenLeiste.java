...
=======
			...
			for (int i = 0; i < fenster.WIDTH; i++) {
				for (int j = 0; j < 8; j++) {
					g.drawImage(hintergrund1, i * Hintergrundpixel, 170 + j
...
>>>>>>> branch 'master' of https://github.com/HindiBones/project.git
			...
			g.setColor(Color.WHITE);
			g.drawString(fenster.spieler.getName(), fenster.BOX + 2,
					itemKy + 75);
			g.drawString("Zeit: "
					+ (System.currentTimeMillis() - fenster.startZeit) / 1000,
					10, 190);
			g.drawString("Level " + (fenster.levelnummer + 1) + "/"
					+ fenster.MAXLEVEL, 100, 190);
			...
			// Sprechblase + Text
			int feld = fenster.Level.getBestimmtenLevelInhalt(
					fenster.spieler.getXPos(), fenster.spieler.getYPos());
			g.setColor(Color.BLACK);
			if (feld == 8) {

				g.drawImage(sblase, 0, itemKy * 2 - 130, null);
				g.drawString(" Nimm den Schluessel", itemKx,
						itemKy * 2 - 130 + 40);
			} else if (feld == 6) {
				if (fenster.spieler.hatSchluessel()) {
					g.drawImage(sblase, 0, itemKy * 2 - 130, null);
					g.drawString("Oeffne die Tuer", itemKx,
							itemKy * 2 - 130 + 40);
				} else {
					g.drawImage(sblase, 0, itemKy * 2 - 130, null);
					g.drawString("Tuer ist verschlossen!", itemKx,
							itemKy * 2 - 130 + 40);
				}
			} else if (feld == 4 || feld == 7) {
				g.drawImage(sblase, 0, itemKy * 2 - 130, null);
