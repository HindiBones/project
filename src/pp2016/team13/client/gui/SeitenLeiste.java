...
/**
 * Seitenleiste beeinhaltet, die Minimap, die Statusleiste und das Chatfenster
 * das von einer anderen Klasse geaddet wird
 * 
...
public class SeitenLeiste extends JPanel {
	...
	private Image boden2, wand2, tuerOffen2, tuerZu2, heiltrank2, schluessel2, john, sblase, hintergrund1,
			heiltrankblau;
	ChatFenster p;
	HindiBones fenster;

	/**
	 * Wird an das Haupfenster gebunden Zeichnet groesstenteils nur
	 * 
	 * @author <Keser, Seyma, 5979919>
	 * @param fenster:
	 *            Wird im HindiBones fenster erstellt
	 */
	...
	public void paint(Graphics g) {
		...
		if (true) {
			...
			for (int i = 0; i < fenster.WIDTH; i++) {
				for (int j = 0; j < fenster.HEIGHT; j++) {
					if (fenster.level.getBestimmtenLevelInhalt(i, j) == 0) {
						...
						g.drawImage(wand2, i * fenster.BOX2, j * fenster.BOX2, null);
						...
						g.drawImage(boden2, i * fenster.BOX2, j * fenster.BOX2, null);
					...
					if (fenster.level.getBestimmtenLevelInhalt(i, j) == 2) {
						...
						g.drawImage(boden2, i * fenster.BOX, j * fenster.BOX, null);
						g.drawImage(tuerOffen2, i * (fenster.BOX2), j * (fenster.BOX2), null);
					}
					if (fenster.level.getBestimmtenLevelInhalt(i, j) == 6) {
						// Geschlossene Tuer gesetzt
						g.drawImage(boden2, i * fenster.BOX, j * fenster.BOX, null);
						g.drawImage(tuerZu2, i * (fenster.BOX2), j * (fenster.BOX2), null);
					}
				}
			...
			g.drawImage(fenster.spieler2.getImage(), fenster.spieler.getXPos() * fenster.BOX2,
					fenster.spieler.getYPos() * fenster.BOX2, null);
			...
			g.drawString("Zeit: " + (System.currentTimeMillis() - fenster.startZeit) / 1000, 10, 190);
			g.drawString("Level " + (fenster.levelnummer + 1) + "/" + fenster.MAXLEVEL, 100, 190);
			...
			g.fillRect(50, itemKy + 80, fenster.spieler.getMaxHealth(), 5);
			g.setColor(Color.GREEN);
			g.fillRect(50, itemKy + 80, fenster.spieler.getLebenspunkte(), 5);

			// Positionierung des Chat panels
			this.add(chatText);
			chatText.setBounds(0, 410, 210, 35);// Groesse+Koord. wird
												// festgelegt
